...
class Objective:
    ...
    def run_trial_old(
        ...
        start = datetime.now()
        earliest_failed_request = self.n_requests + 1
        stop_early = False
        with ThreadPoolExecutor(256) as executor:
                    ...
                    if future.done():
                        ...
                        try:
                            ...
                            if completion_tokens == self.max_completion_tokens:
                                stop_early = True
                                print(f"  Stopping early at {request_id=}")
                                if request_id < earliest_failed_request:
                                    earliest_failed_request = request_id + 1
                                    print(
                                        f"  {temperature:.02f}, {min_p:.02f}, "
                                        + f"{repetition_penalty:.02f}, "
                                        + f"{earliest_failed_request=:}"
                                    )
                for future in done:
                    futures.pop(future)
                if stop_early:
            ...
            for future in as_completed(futures):
                try:
                    ...
                    if completion_tokens == self.max_completion_tokens:
                        print(f"  All requests submitted, but {request_id=:} failed")
                        if request_id < earliest_failed_request:
                            earliest_failed_request = request_id + 1
                            print(
                                f"  {temperature:.02f}, {min_p:.02f}, "
        ...
        if earliest_failed_request < self.n_requests + 1:
            print(f"  Finished with {earliest_failed_request=:}")
        else:
            print("  Finished with all requests completed successfully")

        return repetition_penalty / earliest_failed_request
